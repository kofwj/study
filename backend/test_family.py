...
def main_fn():
    ...
    with TestClient(main.app) as a, TestClient(main.app) as b:
        r = a.post("/api/auth/register", json={"account": "alice", "pin": "1111", "family_name": "A家"})
        ...
        r = b.post("/api/auth/register", json={"account": "bob", "pin": "2222", "family_name": "B家"})
        assert r.status_code == 200, r.text
        ...
        r = b.post("/api/auth/join", json={"account": "carol", "pin": "3333", "code": code, "name": "卡卡"})
        assert r.status_code == 200, r.text
        # carol 进了 A 家，应看到 A 家奖
        with TestClient(main.app) as c:
            r = c.post("/api/auth/login", json={"account": "carol", "pin": "3333"})
            assert r.status_code == 200
