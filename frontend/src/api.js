...
export const api = {
  ...
  bank: () => j('/api/bank'),
  bankDeposit: (amount, days) => j('/api/bank/deposit', { method: 'POST', ...body(days ? { amount, days } : { amount }) }),
  bankWithdraw: (amount) => j('/api/bank/withdraw', { method: 'POST', ...body({ amount }) }),
  bankBreakDeposit: (id) => j(`/api/bank/deposits/${encodeURIComponent(id)}/break`, { method: 'POST' }),
  ...
  admin: {
    ...
    bankInterestConfig: () => j('/api/admin/bank/interest'),
    saveBankInterestConfig: (o) => j('/api/admin/bank/interest', { method: 'PUT', ...body(o) }),
    settleInterestNow: () => j('/api/admin/bank/settle-now', { method: 'POST' }),
