...
| ③ 学习留痕（核心） | 今日打卡→场景里那架**免费**风车转；单元完成→纸飞机从树屋飞到天台（1 架/条）；单词 session→夜空一颗星；到期复习空→**底图月亮加光晕**（不另画一颗月） | Forest 林子；Pikmin 花径 | **做** | 只读、跨天清零；**不把风车/纸飞机再卖一遍** |
...
- `review_clear`：`_due_queue` 为空 → 底图月亮加一圈光晕（**不另画 ☾**）
...
- 前端只在孩子**点进秘密基地 / 图鉴**时展示（场景标题下的「昨晚报」卡片，不挡今日任务），展示后点一下 `POST /api/sprites/morning-ack`。同一天不再弹（跨设备也一致）。登录首页不弹。
...
> **锁定**：与 `docs/base-layout-preview.html` 的 `SPEC` 同一套数字（火箭 13,86 w22 / 望远镜 46,99 w16 / 风车 95,70 w12 / 吊床 82,115 w28 / 萤火虫瓶 20,34 w11 / 奖状 45,43 w10 / 小旗 11,87 w10 / 时间胶囊 74,42 w6 / 热气球 19,56 w18 / 月亮床 75,82 w22）。`y` 可超过 100（锚点在画布底边以下）。未再拖动前不要改。时间胶囊挂木屋右侧墙，不要挡门口。
...
| `memo-capsule` | 回忆 | 74 | 42 | 6 | 不坐。挂在木屋**右侧墙**（窗户旁），不要挡门口 |
