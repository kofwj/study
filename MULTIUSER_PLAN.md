...
**当前范围以 `PLAN.md`「支线 B」为准**：先做全家今日面板和周报全家对比，其次可选扣分（默认关），然后家庭共同目标。榜、星、计时、红包不再作为当前范围。

扣分/负分、多娃排行榜、家长汇总面板（全家对比）、家庭共同任务、长辈红包、专注计时/星级（候选）。§7.5 原文保留作对照，不整包搬进当前迭代。
...
> **优先级建议（已收口，2026-09）**：当前范围以 `PLAN.md`「支线 B」为准。先做全家今日面板 + 周报全家对比（对应 1a/1b 的最小集）；扣分作第二刀且默认可关；家庭共同目标作第三刀。**不做**排行榜/周冠军、星级、专注计时、长辈红包、月报 CSV。P4.3/P4.4 等要开放给其他家庭或老师时再启动。

---

## 7.6 学习科学纵深 + 多版本教材（方案 2026-09）

> 定位一句话：**不做通用题库，只做家长指定的短期冲刺题库**（泛化的刷题是作业帮/小猿的主场，硬碰必死）；做的是它们视野外的**补集**——孩子**线下**（作业本/听写/默写/背诵）哪里薄弱、何时该再练，结构化记下来、到点提醒。底盘是**多版本教材**（先做全江苏，架构预留其他省）。
...
- **复习队列只提醒「去做你练习册上的这件事」，不替你出题、不替你判对错**——这是和作业帮最后一线差异：它把你焊在屏幕里刷题，我们提醒你回到纸上。**唯一的例外是家长自己指定的内容**（英语单词、冲刺题库，见 7.6）：题是家长给的，不是我们推的。
...
## 7.8a M3 细化规格（收口：孩子端呈现 + 复习收口进盯点）

> 经一轮小红书/抖音竞品调研（6 款同类工具对标），M3 从抽象的「丰富清单」落成 3 块可开工规格。多轮迭代时只照这三块做，不堆不飘。

###对标表（6 款，抄 4 不抄 3）

| 工具 |类型| 孩子端呈现 |激励| 家长端 |学/不学 |
|---|---|---|---|---|---|
| 小猿口算 |口算练习 |对/错即时反馈，一局一结算 |积分+勋章拼图+PK 对战 |自动报告 |反馈即时性可学；PK/排行不学 |
| 作业小达人 |作业打卡 |AI 拆任务（练字一张/背一篇） |多孩独立日历+积分 |多娃管理 |多孩独立=P2 已有；AI 拆任务不学 |
| 错题plus |错题整理 |拍照→薄弱知识点自动打标签 |做题积分 |薄弱定位分析 |薄弱点=具体能力点=考点精标同思路；拍照判题不学 |
| 小红花 |习惯打卡 |任务清单+打卡 |自定义积分+兑换+多阶段模板 |日/周/月报告 |周报告正向总结可学（我们只有报问题） |
| 家务表 Family Rewards |家务积分 |全家积分池+抢任务 |徽章+实物兑换 |全家看板 |抢任务不学（孩子内耗）；徽章/兑换已近（阳光+宝箱） |
| 艾宾浩斯打卡表 |复习节奏（纸质爆款） |墙上贴表，看得见第几天/第几遍 |贴纸/涂格 |家长打印管理 |复习可视化可学——[1,3,7,14,30] 翻译成孩子看得懂的轮次 |

###抄这 4 条（进实现）

1. **复习进度可视化**（艾宾浩斯）：今日复习卡显示「第 N 轮」（第几遍），interval_idx→轮次（idx0=第 1 轮），有「越学越牢」的进度感。
2. **薄弱点=具体动作**（错题plus）：考点精标已是具体能力点，孩子端文案转儿童向「再练练竖式进位」，「给动作不给评价」（铁律②已定）。
3. **家长正向信号**（小红花）：盯点卡补一句「本周练牢：X、Y」（本周过的弱项），家长看到「盯有成果」，不比只报问题更愿意用。
4. **即时反馈已就位**（小猿）：+5 阳光飘出+升级庆祝已是等价物，M3 不再加新激励，别堆．

###不抄这 3 条（防手痒）

1. **排行榜/PK/抢任务**：社交竞争留人强，但给孩子横向焦虑，跟「个人家庭+反堆砌」冲突，永不引入（等级升级已是荣誉）。
2. **AI 判题/搜题/自动拆任务**：定位「家长记录薄弱点，**判对错靠家长给的答案，不靠 AI**」，不做．题只来自家长指定（英语单词按标准答案比对、冲刺题库自评+家长抽查），不生成题、不搜题．
3. **错题扣分/负反馈**：只允许「取消勾选扣回」（撤销语义，不是惩罚），不引入错题扣分．

###实施分解（3 块）

**M3.1 今日复习卡（轮次可视化，纯前端）**
- 孩子端「今日复习」卡（App.vue reviewDue）title 前加轮次 pill：`第 {{ interval_idx+1 }} 轮`（idx0=第 1 轮，封顶第 5 轮=再过关就练牢）。数据已有（_wp_rows 返回 interval_idx），无后端改动。
- detail「翻练习册再做一遍，让家长看过关。」不动，不新增文案．

**M3.2 复习到期并入本周盯点 + 正向总结（后端）**
- `build_insights` 优先级最前加 `_insight_review_due(c, kid)`（到期未处理的空弱项（review_due_at<=today and status='open'），顶到最高优先。
- 「本周练牢」正向总结：查本周（>=周一）`status='resolved'`（或升到 30 天档）的 weak_points，取 unit/tag 名，拼一句「本周练牢：竖式进位、like doing」。
- Admin 盯点卡渲染对应字段；阈值不动（复用现有 rules）.

**M3.3 单元头状态行 + 任务卡角标（补全，纯前端）**
- 单元头已有「再练练」+「薄弱：…」标签（App.vue 531-532），补「该复习了」标（该 unit 有到期弱项时，来自 reviewDue 数组前端推导，，无后端改动）。
- 任务卡角标圆点：有薄弱点的 unit，其每张课卡加一个小圆点（unit 级精度，用户可见度适中）。

###接线
- M3.1/M3.3 纯前端数据已有 → 先做。
- M3.2 后端 build_insights 扩展 → 并行或后做。
- 全部不引新表、不加新阈值（复用现有 rules）。
