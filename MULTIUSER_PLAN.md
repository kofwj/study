...
1. **P4.6a** 结论引擎骨架 + 测试低分 / 连击 / 完成量三类（纯现有数据）。
2. **P4.6b** `fitness_standards` 表 + 体测弱项结论（江苏标准人工录）。
3. **P4.6c** 家长端「本周盯点」卡 + 周报整合。
4. **P4.6d（等 7.6）** 复习到期结论并入，优先级顶到最高。
...
1. **P4.7a** 单元头状态行（测试分已有，加薄弱标签 + 该复习），纯前端 + tests 数据。
2. **P4.7b** 运动卡达标进度条（等 fitness_standards 建好）。
3. **P4.7c** 「今日复习」卡 + 孩子版提醒（等 7.6 的 weak_points + 7.7 引擎）。
