#!/usr/bin/env bash
# SSR 渲染体检：把壳 / 子组件用桩数据挂载一次，抓 Vue 警告、抛错和关键文案。
# 两边都覆盖：家长端（Admin* + AdminWords）和孩子端 P4-b 那两件（WordGameCard 入口卡、DailyDialog 弹窗）；
# 弹窗只能验「关着」的空态（不是 props 驱动、SSR 驱动不出打开态，见该桩的注释）。
#
